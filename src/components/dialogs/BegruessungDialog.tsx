...
interface BegruessungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Begruessung['fields']) => Promise<void>;
  defaultValues?: Begruessung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function BegruessungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: BegruessungDialogProps) {
  const [fields, setFields] = useState<Partial<Begruessung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'begrueßung');
      await onSubmit(clean as Begruessung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Begruessung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Begrüßung bearbeiten' : 'Begrüßung hinzufügen';
