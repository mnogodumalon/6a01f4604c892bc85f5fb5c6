...
interface BegruessungViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: Begruessung | null;
  onEdit: (record: Begruessung) => void;
}

export function BegruessungViewDialog({ open, onClose, record, onEdit }: BegruessungViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Begrüßung anzeigen</DialogTitle>
        </DialogHeader>
