...
import type { Begruessung, CreateBegruessung } from '@/types/app';
...
export class LivingAppsService {
  // --- BEGRUESSUNG ---
  static async getBegruessung(): Promise<Begruessung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEGRUESSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Begruessung[];
    return enrichLookupFields(records, 'begrueßung');
  }
  static async getBegruessungEntry(id: string): Promise<Begruessung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEGRUESSUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Begruessung;
    return enrichLookupFields([record], 'begrueßung')[0];
  }
  static async createBegruessungEntry(fields: CreateBegruessung) {
    return callApi('POST', `/apps/${APP_IDS.BEGRUESSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'begrueßung') });
  }
  static async updateBegruessungEntry(id: string, fields: Partial<CreateBegruessung>) {
    return callApi('PATCH', `/apps/${APP_IDS.BEGRUESSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'begrueßung') });
  }
  static async deleteBegruessungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BEGRUESSUNG}/records/${id}`);
  }
