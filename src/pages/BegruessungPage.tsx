...
export default function BegruessungPage() {
  const [records, setRecords] = useState<Begruessung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Begruessung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Begruessung | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Begruessung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getBegruessung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Begruessung['fields']) {
    await LivingAppsService.createBegruessungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Begruessung['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateBegruessungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Begrüßung"
      subtitle={`${records.length} Begrüßung im System`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Begrüßung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
