...
import { BegruessungViewDialog } from '@/components/dialogs/BegruessungViewDialog';
...
const ENTITY_TABS = [
  { key: 'begrueßung', label: 'Begrüßung', pascal: 'Begruessung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('begrueßung');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'begrueßung': return (data as any).begruessung as Begruessung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'begrueßung': return BEGRUESSUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'begrueßung': return {
        create: (fields: any) => LivingAppsService.createBegruessungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBegruessungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBegruessungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'begrueßung' || dialogState?.entity === 'begrueßung') && (
        <BegruessungDialog
          open={createEntity === 'begrueßung' || dialogState?.entity === 'begrueßung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'begrueßung' ? handleUpdate : (fields: any) => handleCreate('begrueßung', fields)}
          defaultValues={dialogState?.entity === 'begrueßung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Begruessung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Begruessung']}
        />
      )}
      {viewState?.entity === 'begrueßung' && (
        <BegruessungViewDialog
          open={viewState?.entity === 'begrueßung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'begrueßung', record: r }); }}
        />
      )}
